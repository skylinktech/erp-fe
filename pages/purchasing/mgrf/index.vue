        ...
        <div class="content-wrapper">
            ...
            <div v-if="isInitialLoading" class="d-flex justify-content-center align-items-center" style="min-height: 400px;">
                <div class="text-center">
                    ...
                    <p class="mt-3 text-muted">Memuat data MGRF...</p>
                </div>
            </div>
            ...
            <div v-else class="container-xxl flex-grow-1 container-pt-12">
                            ...
                            <div class="card-body">
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    <div class="account-heading">
                                        ...
                                        <span class="text-muted">MGRF terdaftar</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="row g-6">
                    <div class="col-12">
                        <h4 class="mt-6 mb-1">Filter MGRF</h4>
                        <p class="mb-0">Temukan semua MGRF perusahaan Anda</p>
                    </div>
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    <button
                                        v-if="userHasRole('superadmin') || userHasPermission('create_mgrf')"
                                        @click="mgrfStore.openModal(null)"
                            ...
                            <div class="card-datatable table-responsive py-3 px-3">
                                ...
                                >
                                    ...
                                    <Column field="noMgrf" header="No. MGRF" :sortable="true" class="text-nowrap">
                                        <template #body="slotProps">
                                            <a 
                                                @click="navigateTo(`/purchasing/mgrf/detail/${slotProps.data.id}`)" 
                                                style="cursor: pointer; color: #666bff; text-decoration: underline;"
                                                title="View detail"
                                                class="text-primary"
                                            >
                                                {{ slotProps.data.noMgrf || '-' }}
                                            </a>
                                        </template>
                                    </Column>
                                    <Column field="iro.noIro" header="IRO" :sortable="true" class="text-nowrap">
                                        <template #body="slotProps">
                                            <a 
                                                @click="navigateTo(`/order-process/iro/detail/${slotProps.data.iro.id}`)" 
                                                style="cursor: pointer; color: #666bff; text-decoration: underline;"
                                                title="View detail"
                                                class="text-primary"
                                            >
                                                {{ slotProps.data.iro?.noIro || '-' }}
                                            </a>
                                        </template>
                                    </Column>
                                            ...
                                            <div class="dropdown d-inline-block">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end mgrf-actions-dropdown">
                                                    ...
                                                    <li v-if="(userHasRole('superadmin') || userHasPermission('approve_mgrf')) && slotProps.data.status === 'pending'">
                                                        <a class="dropdown-item" href="javascript:void(0)" @click="mgrfStore.approveMgrf(slotProps.data.id)">
                                                            <i class="ri-check-line me-2"></i> Approve
                                                        </a>
                                                    </li>
                                                    <li v-if="(userHasRole('superadmin') || userHasPermission('reject_mgrf')) && slotProps.data.status === 'pending'">
                                                        <a class="dropdown-item" href="javascript:void(0)" @click="mgrfStore.rejectMgrf(slotProps.data.id)">
                                                    ...
                                                    <li v-if="(userHasRole('superadmin') || userHasPermission('delete_mgrf')) && slotProps.data.status === 'draft'">
                                                        <a class="dropdown-item text-danger" href="javascript:void(0)" @click="mgrfStore.deleteMgrf(slotProps.data.id)">
                        ...
                        <form @submit.prevent="handleSubmit" novalidate>
                            ...
                            <div class="tab-content pt-6">
                                ...
                                <div class="tab-pane fade" id="form-tabs-items" role="tabpanel">
                                    ...
                                    <div class="mb-4" v-if="form.mgrfItems && form.mgrfItems.length > 0 && form.iroId">
                                        <div class="form-check">
                                            <input 
                                                class="form-check-input" 
                                                type="checkbox" 
                                                :id="'enable-additional'" 
                                                v-model="mgrfStore.enableAdditional"
                                            >
                                            <label class="form-check-label" for="enable-additional">
                                                Item Tambahan (Additional)
                                            </label>
                                        </div>
                                        <small class="text-muted d-block mt-1">Centang untuk menambahkan item tambahan di luar item dari IRO</small>
                                    </div>
                                    
                                    <div v-for="(item, index) in form.mgrfItems" :key="index" class="repeater-item mb-4">
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-12 d-flex justify-content-end">
                                                <button @click.prevent="mgrfStore.removeItem(index)" class="btn btn-outline-danger">Hapus</button>
                                            </div>
                                        </div>
                                        <hr class="my-4">
                                    </div>
                                    <div class="mt-4" v-if="mgrfStore.enableAdditional">
                                        <button @click.prevent="mgrfStore.addItem(true)" class="btn btn-primary">
                            ...
                            <div class="modal-footer mt-6">
                                <button type="button" class="btn btn-outline-secondary" @click="mgrfStore.closeModal()">Tutup</button>
...
const fetchIrosForSelect = async () => {
    const { $api } = useNuxtApp()
    try {
        const res = await fetch(`${$api.iro()}?page=1&rows=500&status=approved`, {
        ...
        console.error('Error fetching IROs for select:', e)
    }
}
...
const modalTitle = computed(() => isEditMode.value ? 'Edit MGRF' : 'Tambah MGRF')
const modalDescription = computed(() => isEditMode.value ? 'Silakan ubah data MGRF di bawah ini.' : 'Silakan isi form di bawah ini untuk menambahkan data MGRF baru.')
...
const totalAmount = computed(() => {
    if (!form.value || !form.value.mgrfItems) return 0
    return form.value.mgrfItems.reduce((sum, item) => sum + (item.subtotal || 0), 0)
})
...
const { isLoading: isDataLoading, error: dataError } = usePageData({
    ...
    loaders: [
        ...
        () => fetchIrosForSelect(),
        () => fetchUnitsForSelect(),
        () => mgrfStore.fetchMgrfs(),
        () => mgrfStore.fetchStats(),
    ],
    onSuccess: () => {
        setListTitle('MGRF', stats.value.total || 0)
    },
    waitAll: true
})
...
onMounted(() => {
    const modalElement = document.getElementById('MgrfModal')
    if (modalElement) {
    ...
    if (editId && typeof editId === 'string') {
        nextTick(() => mgrfStore.openModal({ id: editId }))
    }
})
...
const debouncedSearch = useDebounceFn(() => {
    mgrfStore.setSearch(globalFilterValue.value)
}, 500)
...
watch(filters, (newFilters) => {
    ...
    mgrfStore.setFilters(restFilters)
}, { deep: true })
...
const onPage = (event) => {
    if (event) {
        ...
        mgrfStore.setPagination(validEvent)
    }
}
...
const handleSubmit = () => {
    ...
    if (!form.value.mgrfItems || form.value.mgrfItems.length === 0) {
        useToast().error({
            title: 'Error',
    ...
    const invalidItems = form.value.mgrfItems.filter((item, index) => {
    ...
    form.value.mgrfItems.forEach((item) => {
...
const onIroChange = async (iroId) => {
    if (!iroId || isEditMode.value) return
    
    const toast = useToast()
    const { $api } = useNuxtApp()
    
    try {
        // Fetch IRO details dengan includeDetails
        const response = await fetch(`${$api.iro()}/${iroId}`, {
            headers: { 'Accept': 'application/json' },
            credentials: 'include'
        })
        
        if (!response.ok) {
            throw new Error('Gagal mengambil data IRO')
        }
        
        const result = await response.json()
        const iroData = result.data
        
        if (!iroData || !iroData.iroDetails) {
            return
        }
        
        // Filter items yang itemType === 'PRODUCT'
        const productItems = iroData.iroDetails.filter((detail) => {
            const itemType = String(detail.itemType || detail.item_type || '').toUpperCase()
            return itemType === 'PRODUCT' && (detail.productId || detail.product_id || detail.product?.id)
        })
        
        if (productItems.length === 0) {
            toast.info({
                title: 'Info',
                message: 'IRO yang dipilih tidak memiliki produk',
                color: 'blue'
            })
            return
        }
        
        // Map ke format mgrfItems
        const mappedItems = productItems.map((detail) => {
            const productId = detail.productId || detail.product_id || detail.product?.id
            const product = products.value.find(p => p.id === productId)
            
            return {
                productId: productId,
                warehouseId: null, // User harus pilih manual
                unitId: product?.unitId || null,
                quantity: Number(detail.quantity) || 1,
                price: Number(detail.price) || 0,
                subtotal: Number(detail.subtotal) || (Number(detail.quantity) || 1) * (Number(detail.price) || 0),
                additional: false, // Items dari IRO default false
                description: detail.description || ''
            }
        })
        
        // Clear existing items dan set new items
        form.value.mgrfItems = mappedItems
        
        toast.success({
            title: 'Success',
            message: `${mappedItems.length} produk berhasil dimuat dari IRO`,
            color: 'green',
            position: 'topRight',
            layout: 2,
        })
      } catch (error) {
          console.error('Error loading IRO details:', error)
          const errorMessage = error instanceof Error ? error.message : 'Gagal memuat data IRO'
          toast.error({
              title: 'Error',
...
const onProductChange = (index) => {
    const item = form.value.mgrfItems[index]
...
const clearFilters = () => {
    filters.value.iroId = null
    filters.value.status = null
    filters.value.startDate = null
    filters.value.endDate = null
    mgrfStore.setFilters(filters.value)
}
...
definePageMeta({
    ...
    title: 'MGRF',
    description: 'MGRF Management',
    keywords: 'MGRF, Purchasing, Kainnova Digital Solutions',
