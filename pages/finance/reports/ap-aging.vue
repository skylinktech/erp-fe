  ...
  <div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-pt-12">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h4 class="mb-1">AP Aging</h4>
          <p class="mb-0 text-muted">Hutang outstanding per bucket umur (as of {{ report?.asOf || '—' }})</p>
        </div>
...
async function load() {
  ...
  try {
    ...
    const res = await fetch($api.apAgingReport(qs.toString()), {
      headers: { Accept: 'application/json' },
      credentials: 'include',
    })
    const json = await res.json()
    if (!res.ok) throw new Error(json.message || 'Gagal memuat AP aging')
