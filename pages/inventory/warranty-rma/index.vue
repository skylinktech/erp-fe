<template>
  <div class="content-wrapper">
    <div class="container-xxl flex-grow-1">
      <p class="mb-6">Physical shipment only — RMA_SHIP via InventoryPostingService.</p>
      ...
      <div class="card">
        <ListPageTableHeader
          ...
          search-placeholder="Cari RMA / tracking / equipment..."
          ...
          @export="(f) => exportData(f, myDataTableRef, { title: 'Warranty RMA' })"
        >
          <template #toolbar-extra>
            <NuxtLink to="/inventory/warranty-claim" class="btn btn-outline-secondary">Claims</NuxtLink>
          </template>
        </ListPageTableHeader>
        ...
        <div class="card-datatable table-responsive py-3 px-3">
          ...
          >
            ...
            <Column field="rmaNo" header="RMA" :sortable="true" :show-filter-menu="false">
              <template #body="{ data }">
                <NuxtLink :to="`/inventory/warranty-rma/${data.id}`">
                  <code>{{ data.rmaNo }}</code>
                </NuxtLink>
              </template>
              <template #filter="slotProps">
                <ListColumnFilter v-bind="slotProps" placeholder="RMA-" />
              </template>
            </Column>
            <Column field="claim.claimNo" header="Claim" :sortable="true" :show-filter-menu="false">
              <template #body="{ data }">{{ data.claim?.claimNo }}</template>
              <template #filter="slotProps">
            ...
            <Column field="sourceWarehouse.code" header="Warehouse" :sortable="true" :show-filter-menu="false">
            ...
            <Column field="shippedAt" header="Shipped" :sortable="true" :show-filter-menu="false">
              <template #body="{ data }">{{ formatDate(data.shippedAt) }}</template>
            </Column>
          </MyDataTable>
        </div>
      </div>
...
definePageMeta({ middleware: ['auth', 'check-permission'], title: 'Warranty RMA' })
